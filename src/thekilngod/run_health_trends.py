#!/usr/bin/env python3
"""Analyze and visualize kiln run-health trend history."""
...
DEFAULT_HISTORY = os.path.join(REPO_DIR, "storage", "logs", "run-health-history.jsonl")
DEFAULT_EXCLUSIONS = os.path.join(REPO_DIR, "storage", "logs", "run-health-exclusions.json")
try:
    import config
...
def load_history(path: str) -> list[dict[str, Any]]:
    """Load JSONL history rows from disk.

    Invalid lines are ignored so one corrupted append does not block analysis.
    """
...
def load_exclusions(path: str) -> set[str]:
    """Load excluded run IDs used to hide intentional outliers."""
    if not os.path.exists(path):
        return set()
    try:
        with open(path, encoding="utf-8") as f:
...
def print_summary(rows: list[dict[str, Any]]) -> None:
    """Print human-readable high-level trend metrics."""
    if not rows:
    ...
    print(
        f"Last run: profile={last.get('profile')} "
        f"reason={last.get('reason')} ended_at={last.get('ended_at')}"
    )
...
def warning_analysis(rows: list[dict[str, Any]]) -> str:
    """Return heuristic warning text based on recent run drift."""
    if len(rows) < 6:
    ...
    if gap_rising and duty_rising:
        return (
            "WARNING: Trend suggests declining element performance "
...
def plot(rows: list[dict[str, Any]], output_file: str) -> int:
    """Render run-health trend charts and write them to disk."""
    try:
...
def main() -> int:
    """CLI entrypoint for run-health analytics."""
    parser = argparse.ArgumentParser(description="Plot kiln run health trends across runs.")
    parser.add_argument(
        "--history",
        default=DEFAULT_HISTORY,
        help="Path to run health history JSONL",
    )
    parser.add_argument(
        "--exclusions",
        default=DEFAULT_EXCLUSIONS,
        help="Path to exclusion JSON list",
    )
    parser.add_argument(
        "--include-excluded",
        action="store_true",
        help="Include runs marked excluded",
    )
    parser.add_argument("--out", default="run-health-trends.png", help="Output image file")
    parser.add_argument(
        "--print-only",
        action="store_true",
        help="Print trend summary only, do not plot",
    )
